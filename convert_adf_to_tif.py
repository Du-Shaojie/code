...
# conda虚拟环境的路径,windows系统下,如果为mac系统,请修改为mac系统下的路径
envs_path = r'D:\programs\anaconda\envs\envs311'  # 需要修改为自己的虚拟环境的路径
proj_lib_path = os.path.join(envs_path, r'Library\share\proj')
os.environ['PROJ_LIB'] = proj_lib_path  # 动态添加PROJ_LIB的搜索路径，防止出现proj.db找不到的情况
...
if __name__ == '__main__':
    root_source_path = r'G:\月100M植被指数'  # adf源数据的目录
    root_save_path = r'E:\NDVI'  # 数据转换后保存的目录，如果不存在则创建
    print("adf文件的根路径：", root_source_path)
    print("完成tif文件切割后的保存路径：", root_save_path)
